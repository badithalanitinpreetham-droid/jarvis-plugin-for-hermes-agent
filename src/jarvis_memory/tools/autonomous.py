...
class AutonomousExecutor:
    ...
    @staticmethod
    def _failed_ids(state: Dict[str, Any]) -> set[str]:
        return {str(x) for x in state.get("failed_steps", [])}

    @staticmethod
    def _cancelled_ids(state: Dict[str, Any]) -> set[str]:
        return {
            str(h.get("step_id")) for h in state.get("history", [])
            if h.get("status") in {"cancelled_by_race_winner", "skipped_duplicate"}
        }

    def _terminal_ids(self, state: Dict[str, Any]) -> set[str]:
        return self._completed_ids(state) | self._failed_ids(state) | self._cancelled_ids(state)
    ...
    def _pending_indices(self, state: Dict[str, Any]) -> List[int]:
        terminal = self._terminal_ids(state)
        return [
            i for i, step in enumerate(state.get("plan", {}).get("steps", []))
            if str(step.get("id")) not in terminal
        ]
    ...
    def _race_members(self, state: Dict[str, Any], idx: int) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            if step.get("race_group_id") == group
            and str(step.get("id")) not in terminal
            and self._dependencies_satisfied(state, step)
        ]
    ...
    def _parallel_members(self, state: Dict[str, Any], idx: int) -> List[Dict[str, Any]]:
        ...
        if group:
            return [
                steps[i] for i in ready
                if steps[i].get("execution_mode") == "parallel"
                and steps[i].get("parallel_group_id") == group
            ]
    ...
    def _active_members(self, state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            step for step in state.get("plan", {}).get("steps", [])
            if str(step.get("id")) in ids
        ]
    ...
    def _status_response(self, workflow_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed": len(self._failed_ids(state)),
    ...
    def _approval_payload(self, workflow_id: str, members: List[Dict[str, Any]]) -> Dict[str, Any]:
        missing = [
            step for step in members
            if str(step.get("id")) not in {str(x) for x in self._get_state(workflow_id).get("approved_step_ids", [])}
        ]
        payload: Dict[str, Any] = {
            "workflow_id": workflow_id,
            "status": "awaiting_approval",
            "requires_approval": True,
            "approval_reason": "; ".join(sorted({self._get_approval_reason(step) for step in missing})) or "Approval required",
        }
        if len(members) > 1:
            payload["parallel_steps"] = members
            payload["approval_scope"] = "Approve the batch to release the listed Hermes work items."
        else:
    ...
    def get_next_step(self, workflow_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if active:
                members = [
                    step for step in active
                    if str(step.get("id")) not in self._terminal_ids(state)
                ]
                if members:
                    if state.get("status") == "awaiting_approval":
                        approved = {str(x) for x in state.get("approved_step_ids", [])}
                        missing = [step for step in members if str(step.get("id")) not in approved]
                        if missing:
                            self._persist(workflow_id)
                            return self._approval_payload(workflow_id, members)
            ...
            if not ready:
                ...
                return {
                    "workflow_id": workflow_id,
                    "status": "blocked",
                    "message": "No pending step has its dependencies satisfied.",
                }
            ...
            if not members:
                state["status"] = "blocked"
                self._persist(workflow_id)
                return {"workflow_id": workflow_id, "status": "blocked", "message": "No executable member remains."}

            approved = {str(x) for x in state.get("approved_step_ids", [])}
            missing = [step for step in members if str(step.get("id")) not in approved]
            if any(self._requires_approval(step) for step in members) and missing:
                state["active_batch"] = [step.get("id") for step in members]
                state["status"] = "awaiting_approval"
                state["approval_requested_at"] = state.get("approval_requested_at") or self._now_iso()
                state["step_started_at"] = None
                self._persist(workflow_id)
                return self._approval_payload(workflow_id, members)

            state["active_batch"] = [step.get("id") for step in members] if len(members) > 1 else []
    ...
    def approve_step(self, workflow_id: str, step_id: Any) -> Dict[str, Any]:
        with self._lock:
            ...
            if not active:
                ...
                active = (
                    self._race_members(state, idx)
                    if step.get("execution_mode") == "race"
                    else self._parallel_members(state, idx)
                    if step.get("execution_mode") == "parallel"
                    else [step]
                )
                state["active_batch"] = [item.get("id") for item in active]
                state["status"] = "awaiting_approval"
            ids = {str(step.get("id")) for step in active}
            if str(step_id) not in ids:
                return {"error": f"Step {step_id} is not currently pending"}
            approved = state.setdefault("approved_step_ids", [])
            if len(active) > 1:
                approved.extend(step.get("id") for step in active)
            else:
                approved.append(step_id)
            state["approved_step_ids"] = list(dict.fromkeys(approved))
        ...
        with self._lock:
            ...
            if not active:
                ...
                active = (
                    ...
                    if current.get("execution_mode") == "parallel"
                    else [current]
                )
            target = next((step for step in active if str(step.get("id")) == str(step_id)), None)
            if target is None:
                if any(str(h.get("step_id")) == str(step_id) for h in state.get("history", [])):
                    return {"error": f"Step {step_id} has already been reported"}
                return {"error": f"Step {step_id} is not currently expected"}
            if any(
                str(h.get("step_id")) == str(step_id)
                and h.get("status") in VALID_RESULT_STATUSES
            ...
            if status == "success":
                ...
                if is_race:
                    for loser in active:
                        ...
                        if str(loser_id) != str(step_id) and str(loser_id) not in self._terminal_ids(state):
                            state.setdefault("history", []).append({
                                "step_id": loser_id,
            ...
            if len(active) > 1 and not is_race:
                active_ids = {str(step.get("id")) for step in active}
                reported = {
                    str(item.get("step_id")): item.get("status")
                    for item in state.get("history", [])
                    if str(item.get("step_id")) in active_ids
                    and item.get("status") in VALID_RESULT_STATUSES
                }
                finished = all(
                    str(step.get("id")) in self._completed_ids(state)
                    or str(step.get("id")) in self._failed_ids(state)
                    or str(step.get("id")) not in reported
                    for step in active
                ) and all(str(step.get("id")) in reported for step in active)
                if not finished:
                    ...
                    return {
                        ...
                        "parallel": True,
                        "pending_step_ids": [
                            step.get("id") for step in active
                            if str(step.get("id")) not in self._terminal_ids(state)
                        ],
                    }
            ...
            failed_active = [
                ...
                if str(item.get("id")) in self._failed_ids(state)
            ]
            if failed_active and self.planner and state.get("replan_count", 0) < self.replan_max_retries:
                return self._auto_replan(workflow_id, failed_active[0], error or "Workflow step failed")
            if failed_active and not self.planner:
    ...
    def _record_memory(self, state: Dict[str, Any], step: Dict[str, Any], status: str, error: Optional[str]) -> None:
        ...
        try:
            text = (
                f"Workflow {state.get('plan', {}).get('goal', '')} step {step.get('id')} "
                f"({step.get('action')}): {status}"
            )
            if error:
                text += f" — {error}"
            self.memory_engine.add_memory(
                ...
                text,
                {
                    "type": "execution_log" if status == "success" else "error_log",
        ...
        with self._lock:
            ...
            try:
                ...
                new_plan = self.planner.replan(
                    state["plan"], failed_step.get("id"), error, state.get("history", [])
                )
    ...
    def cancel_workflow(self, workflow_id: str, reason: str = "") -> Dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "completed_steps": len(self._completed_ids(state)),
                "total_steps": len(state.get("plan", {}).get("steps", [])),
            }
    ...
    def reflect(self, workflow_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            successes = [record for record in history if record.get("status") == "success"]
            failures = [record for record in history if record.get("status") == "failed"]
            successful_tools = sorted({record.get("tool") for record in successes if record.get("tool")})
            failed_tools = sorted({record.get("tool") for record in failures if record.get("tool")})
    ...
    def self_evolve(self, profile_id: str) -> Dict[str, Any]:
        states = {
            wid: state for wid, state in self.store.load_all().items()
        ...
        reliable = sorted(
            (
                tool for tool, count in tool_successes.items()
                if count >= 2 and tool_failures.get(tool, 0) == 0
            ),
            key=tool_successes.get,
            reverse=True,
        )
        for tool in unreliable:
            ...
            if failures >= 3 and successes / max(1, successes + failures) < 0.2:
                self.store.mark_tool_broken(
                    tool,
                    f"Failed {failures} times; auto-flagged by self-evolution",
                )
        completed = sum(1 for state in states.values() if state.get("status") == "completed")
        return {
            "profile_id": profile_id,
            "total_workflows": len(states),
            "completion_rate": round(completed / len(states), 2),
            "total_replans": sum(state.get("replan_count", 0) for state in states.values()),
