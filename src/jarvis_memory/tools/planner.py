...
def _extract_bot_roster(context: str) -> tuple[str, List[Dict[str, Any]]]:
    """Support the legacy string context while accepting Hermes roster JSON."""
    if not context:
        return "", []
    try:
...
class WorkflowPlanner:
    ...
    def _recall_lessons(self, goal: str, profile_id: str) -> List[str]:
        ...
        try:
            ...
            for memory in memories or []:
                text = (memory.get("text", memory.get("content", memory.get("memory", "")))
                        if isinstance(memory, dict) else str(memory))
                if text and any(marker in text.lower() for marker in ("workflow", "failed", "succeeded", "lesson", "replan")):
    ...
    def create_plan(self, goal: str, profile_id: str, context: str = "",
                    available_bots: Optional[Iterable[Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        roster = list(available_bots) if available_bots is not None else embedded_roster or None
        lessons = self._recall_lessons(goal, profile_id)
        organisation = self.organisation_planner.design(goal, working_context, lessons, roster).as_dict()
        experience = summarize_experience([], lessons).as_dict()
        context_packet = build_context_packet(
            goal=goal, profile_id=profile_id, organisation=organisation,
            lessons=lessons, context=working_context, registry=self.registry,
        )
...
Jarvis organisation: {json.dumps(organisation)}
Past lessons: {json.dumps(lessons)}

Hermes executes the plan. Do not invent tools, Bots, Kanban or APIs. Use the supplied
Bot/profile metadata only to assign work to existing Hermes Bots. Temporary agents may
be requested only where the organisation says they are justified.
...
Output JSON only with goal, steps, estimated_steps and success_criteria."""
        ...
        if error:
            return {"error": f"Invalid plan: {error}", "goal": goal, "steps": [],
                    "fallback_mode": True, "organisation": organisation, "context_packet": context_packet}

        if self.store:
            broken = set(self.store.get_broken_tools())
            used = {step.get("tool") for step in plan["steps"]}
            for tool in sorted(used & broken):
                repair_id = f"repair-{tool}"
                plan["steps"].insert(0, {
                    "id": repair_id, "action": f"Diagnose and repair broken Hermes tool '{tool}'.",
                    "tool": "hermes_code_repair", "parameters": {"tool_name": tool, "reason": "Tool is flagged as broken in Jarvis state."},
                    "confidence": 0.5, "risk": "high", "requires_approval": True,
                })
                # Keep dependency semantics valid when a repair step is inserted.
                for step in plan["steps"][1:]:
                    if step.get("tool") == tool:
                        step.setdefault("depends_on", []).insert(0, repair_id)

        plan.update({"organisation": organisation, "context_packet": context_packet, "experience": experience,
    ...
    def _heuristic_plan(goal: str, lessons: List[str], organisation: Dict[str, Any]) -> Dict[str, Any]:
        assignments = organisation.get("assignments", []) or [{"role": "generalist", "purpose": "Own the goal."}]
        ...
        for index, assignment in enumerate(assignments, start=1):
            ...
            if role in discovery_roles and has_discovery:
                mode = "parallel"
                discovery_ids.append(sid)
            elif role == "writer":
                deps = [role_to_id[r] for r in ("researcher", "analyst") if r in role_to_id]
            elif role == "reviewer":
                deps = [s["id"] for s in steps]
            elif role == "publisher":
                deps = [s["id"] for s in steps if s.get("role") in {"writer", "reviewer", "developer"}]
            elif steps:
                deps = [steps[-1]["id"]]
            ...
            steps.append({
                ...
                "tool": "execute",
                "parameters": {"goal": goal, "role": role, "selected_bot": assignment.get("selected_bot"), "organisation": organisation},
        ...
        return {"goal": goal, "steps": steps, "estimated_steps": len(steps), "success_criteria": "Task completed and verified by Hermes."}

    def replan(self, original_plan: Dict[str, Any], failed_step: Any, error: str,
    ...
    def _heuristic_replan(original_plan: Dict[str, Any], failed_step: Any, error: str) -> Dict[str, Any]:
        ...
        index = next((i for i, step in enumerate(steps) if str(step.get("id")) == str(failed_step)), None)
        if index is None:
            return {
                ...
                "steps": [
                    ...
                    {"id": "recovery-2", "action": "Retry the goal using an alternative approach.", "tool": "execute", "parameters": {"goal": original_plan.get("goal", ""), "retry_reason": error[:500]}, "confidence": 0.6, "risk": "medium", "requires_approval": True, "depends_on": ["recovery-1"]},
                ],
        ...
        failed.update({
            ...
            "parameters": {**failed.get("parameters", {}), "_retry_reason": error[:300], "_alternative": True},
        ...
        for step in steps[index + 1:]:
            if step.get("depends_on") and steps[index].get("id") in step["depends_on"]:
                step = dict(step)
        return {"goal": original_plan.get("goal", ""), "steps": [dict(s) for s in steps[:index]] + [failed] + [dict(s) for s in steps[index + 1:]], "estimated_steps": len(steps), "success_criteria": original_plan.get("success_criteria", "Task completed"), "replan_reason": f"Step {failed_step} failed: {error[:200]}"}
