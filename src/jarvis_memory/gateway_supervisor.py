...
class GatewaySupervisor:
    ...
    def __init__(self, memory_client: TencentMemoryClient, start_cmd: Optional[list] = None,
                 auto_start: Optional[bool] = None, autonomous_executor=None):
    ...
    def _spawn(self) -> None:
        ...
        self._proc = subprocess.Popen(self.start_cmd, cwd=cwd, stdout=subprocess.DEVNULL,
        ...
        while not self._stop_event.wait(self.WATCHDOG_INTERVAL):
            ...
            if self.autonomous_executor:
                try:
                    ...
                    for item in stalled:
                        logger.warning("Stalled workflow %s at step %s (%ss)", item["workflow_id"],
                                       (item.get("stalled_step") or {}).get("id", "?"), item.get("elapsed_seconds", 0))
                except Exception:
    ...
    def _process_triggers(self) -> None:
        ...
        for trigger in store.get_triggers():
            ...
            planner = WorkflowPlanner(memory_engine=self.autonomous_executor.memory_engine, store=store)
            plan = planner.create_plan(trigger["goal"], trigger["profile_id"])
            if plan.get("error"):
            ...
            if result.get("error"):
                ...
                continue
            # Advance only after the plan was accepted into durable workflow state.
    ...
    def _safe_telemetry(self) -> None:
        try:
            ...
            if cpu > 95 or ram > 95:
                ...
                # Never automatically spawn a kill-process workflow under load unless explicitly enabled.
                ...
                if self.autonomous_executor:
                    plan = {"goal": "Diagnose system overload safely", "steps": [{
                        "id": "diagnose-overload", "action": "Diagnose top CPU/RAM consuming processes; do not kill anything without approval.",
                        "tool": "jarvis_os_control", "parameters": {"action": "diagnose_load"},
                        "confidence": 0.95, "risk": "high", "requires_approval": True}],
                        "estimated_steps": 1, "success_criteria": "Root cause identified"}
                    self.autonomous_executor.start_workflow(f"auto-recovery-{uuid.uuid4().hex}", plan, "system_admin_bot")
        except Exception:
    ...
    def status(self) -> dict:
        ...
        return {"auto_start": self.auto_start, "owns_process": self._we_own_process,
                "process_alive": bool(self._proc and self._proc.poll() is None) if self._we_own_process else None,
                "consecutive_misses": self._consecutive_misses, "known_profiles": count}
